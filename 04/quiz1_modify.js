...
let page = 1;
let listNum = 3;
let totalPage = Math.ceil(todayPhoto.length / listNum);
...
function getStartIndex(page) {
  const startIdex = (page - 1) * listNum;
  return startIdex;
}

function printImg(page) {
  const startIdex = getStartIndex(page);
  let str = '';
  for (let i = startIdex; i < startIdex + listNum; i++) {
    if (todayPhoto[i]) {
